...
class ConfigManager {
    ...
    setupEventListeners() {
        // RPC配置
        document.getElementById('testRpcBtn')?.addEventListener('click', () => this.testRpcConnection());
        document.getElementById('saveRpcBtn')?.addEventListener('click', () => this.saveRpcConfig());

        // 代币配置
        document.getElementById('validateTokenBtn')?.addEventListener('click', () => this.validateTokenAddress());
        document.getElementById('saveTokenBtn')?.addEventListener('click', () => this.saveTokenConfig());

        // 倒计时配置
        document.getElementById('resetCountdownBtn')?.addEventListener('click', () => this.resetCountdown());
        document.getElementById('saveCountdownBtn')?.addEventListener('click', () => this.saveCountdownConfig());

        // 持仓倒计时配置
        document.getElementById('resetRewardCountdownBtn')?.addEventListener('click', () => this.resetRewardCountdown());
        document.getElementById('saveRewardCountdownBtn')?.addEventListener('click', () => this.saveRewardCountdownConfig());

        // 持仓快照管理
        document.getElementById('viewHoldersSnapshotsBtn')?.addEventListener('click', () => this.viewHoldersSnapshots());
        document.getElementById('exportHoldersSnapshotsBtn')?.addEventListener('click', () => this.exportHoldersSnapshots());
        document.getElementById('clearHoldersSnapshotsBtn')?.addEventListener('click', () => this.clearHoldersSnapshots());
        ...
        document.getElementById('refreshDetectionBtn')?.addEventListener('click', () => this.refreshDetectionStatus());

        // 交易记录管理
        document.getElementById('viewTransactionsBtn')?.addEventListener('click', () => this.viewTransactions());
        ...
        // 大额交易通知管理
        document.getElementById('viewLargeTransactionsBtn')?.addEventListener('click', () => this.viewLargeTransactions());
        document.getElementById('clearLargeTransactionsBtn')?.addEventListener('click', () => this.clearLargeTransactions());
        document.getElementById('exportLargeTransactionsBtn')?.addEventListener('click', () => this.exportLargeTransactions());

        // 成功地址管理
        document.getElementById('viewSuccessAddressesBtn')?.addEventListener('click', () => this.viewSuccessAddresses());
        document.getElementById('clearSuccessAddressesBtn')?.addEventListener('click', () => this.clearSuccessAddresses());
        document.getElementById('exportSuccessAddressesBtn')?.addEventListener('click', () => this.exportSuccessAddresses());

        // 奖励数据管理
        document.getElementById('viewRewardDataBtn')?.addEventListener('click', () => this.viewRewardData());
        document.getElementById('exportRewardDataBtn')?.addEventListener('click', () => this.exportRewardData());
        document.getElementById('viewRewardHistoryBtn')?.addEventListener('click', () => this.viewRewardHistory());
        document.getElementById('clearRewardDataBtn')?.addEventListener('click', () => this.clearRewardData());

        // 系统操作
        document.getElementById('saveAllBtn')?.addEventListener('click', () => this.saveAllConfig());
    ...
    // 重置倒计时
    resetCountdown() {
        this.showModal('确认重置', '确定要重置倒计时吗？这将重新开始全局倒计时。', () => {
            // 创建新的倒计时时间
            const minutes = parseInt(document.getElementById('countdownMinutes').value) || 5;
            const now = new Date();
            ...
            this.log(`倒计时已重置为${minutes}分钟`, 'success');
        });
    }
